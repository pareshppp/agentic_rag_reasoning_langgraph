...
if __name__ == '__main__':
    # ... (previous test cases for analyze_query_node, planner_node, tool_executor_node remain)
    # Keep existing test setup for MockMessage and dummy files if needed by earlier nodes' tests.

    print("\n--- Testing Response Synthesizer Node ---")

    # Test Case 1: Greeting
    state_greeting_synth: AgentState = {
        "user_query": "Hi",
        "query_type": "greeting",
        "chat_history": [], "retrieved_documents": [], "file_contents": {},
        "missing_info_request": "", "plan": [], "current_step_index": 0,
        "final_answer": "", "intermediate_steps": []
    }
    print("\n--- Test Case: Greeting (Synthesizer) ---")
    synthesized_state_greeting = response_synthesizer_node(state_greeting_synth)
    print(f"  Final Answer: {synthesized_state_greeting['final_answer']}")
    assert "Hello! I am a RAG agent." in synthesized_state_greeting['final_answer']

    # Test Case 2: Ambiguous
    state_ambiguous_synth: AgentState = {
        "user_query": "Tell me about it.",
        "query_type": "ambiguous",
        "missing_info_request": "Could you please specify what 'it' refers to?",
        "chat_history": [], "retrieved_documents": [], "file_contents": {},
        "plan": [], "current_step_index": 0, "final_answer": "", "intermediate_steps": []
    }
    print("\n--- Test Case: Ambiguous (Synthesizer) ---")
    synthesized_state_ambiguous = response_synthesizer_node(state_ambiguous_synth)
    print(f"  Final Answer: {synthesized_state_ambiguous['final_answer']}")
    assert synthesized_state_ambiguous['final_answer'] == "Could you please specify what 'it' refers to?"

    # Test Case 3: Error
    state_error_synth: AgentState = {
        "user_query": "This caused an error.",
        "query_type": "error",
        "missing_info_request": "An internal error occurred during planning.",
        "chat_history": [], "retrieved_documents": [], "file_contents": {},
        "plan": [], "current_step_index": 0, "final_answer": "", "intermediate_steps": []
    }
    print("\n--- Test Case: Error (Synthesizer) ---")
    synthesized_state_error = response_synthesizer_node(state_error_synth)
    print(f"  Final Answer: {synthesized_state_error['final_answer']}")
    assert synthesized_state_error['final_answer'] == "An internal error occurred during planning."
    
    state_error_synth_no_msg: AgentState = {
        "user_query": "This caused an error.",
        "query_type": "error",
        "missing_info_request": "", # Test default error message
        "chat_history": [], "retrieved_documents": [], "file_contents": {},
        "plan": [], "current_step_index": 0, "final_answer": "", "intermediate_steps": []
    }
    print("\n--- Test Case: Error (Synthesizer, no specific message) ---")
    synthesized_state_error_no_msg = response_synthesizer_node(state_error_synth_no_msg)
    print(f"  Final Answer: {synthesized_state_error_no_msg['final_answer']}")
    assert "I encountered an issue processing your request." in synthesized_state_error_no_msg['final_answer']


    # Test Case 4: Synthesis from Document and File Content
    # Requires GOOGLE_API_KEY for LLM call
    # Ensure Document is imported: from langchain_core.documents import Document
    sample_doc = Document(page_content="LangGraph is a library for building stateful, multi-actor applications with LLMs. It is part of the LangChain ecosystem.")
    sample_file_path = "my_notes.txt"
    sample_file_content = "My notes indicate that FAISS is used for vector similarity search."
    
    state_synthesis_data: AgentState = {
        "user_query": "What is LangGraph and how is FAISS related, according to my sources?",
        "query_type": "retrieval", # or 'complex_qa' etc.
        "retrieved_documents": [sample_doc],
        "file_contents": {sample_file_path: sample_file_content},
        "chat_history": [], "missing_info_request": "", "plan": ["Synthesize answer..."], # Plan completed
        "current_step_index": 1, "final_answer": "", "intermediate_steps": [("ToolExecution", "some results")]
    }
    print("\n--- Test Case: Data Synthesis (Synthesizer) ---")
    synthesized_state_data = response_synthesizer_node(state_synthesis_data)
    print(f"  User Query: {state_synthesis_data['user_query']}")
    print(f"  Final Answer: {synthesized_state_data['final_answer']}")
    # Assertions could check for inclusion of "LangGraph" and "FAISS" and reference to sources
    assert "LangGraph" in synthesized_state_data['final_answer']
    assert "FAISS" in synthesized_state_data['final_answer']
    assert "Document 1" in synthesized_state_data['final_answer'] or "LangGraph is a library" in synthesized_state_data['final_answer']
    assert f"file '{sample_file_path}'" in synthesized_state_data['final_answer'] or "FAISS is used" in synthesized_state_data['final_answer']

    # Test Case 5: Insufficient Information
    state_synthesis_insufficient: AgentState = {
        "user_query": "What is the weather like on Mars?",
        "query_type": "retrieval",
        "retrieved_documents": [sample_doc], # Contains info about LangGraph, not Mars
        "file_contents": {},
        "chat_history": [], "missing_info_request": "", "plan": ["Synthesize answer..."],
        "current_step_index": 1, "final_answer": "", "intermediate_steps": []
    }
    print("\n--- Test Case: Insufficient Data Synthesis (Synthesizer) ---")
    synthesized_state_insufficient = response_synthesizer_node(state_synthesis_insufficient)
    print(f"  User Query: {state_synthesis_insufficient['user_query']}")
    print(f"  Final Answer: {synthesized_state_insufficient['final_answer']}")
    assert "cannot answer" in synthesized_state_insufficient['final_answer'].lower() or \
           "based on the provided documents and files" in synthesized_state_insufficient['final_answer'].lower() or \
           "no relevant information" in synthesized_state_insufficient['final_answer'].lower()


    print("\nAll Node testing (including Synthesizer) complete.")
